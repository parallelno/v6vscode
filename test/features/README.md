...
This scenario verifies Extension Host startup and a real C `add8 -> accumulate -> main` Call Stack from the checked-in `test/fixtures/cdbg` probe. It asserts semantic frame names and order after stopping in `add8`. Each frame remains valid only for the current stopped generation. The runner must fail explicitly when the emulator or fixture is unavailable. A successful run writes `test/features/debug-adapter/result.txt`; failed or partial runs must not update that file.
