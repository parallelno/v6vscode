...
Source debugging loads a final companion ELF through `debug-artifact-loader.ts`. The immutable debug index maps source lines to CPU addresses and CPU addresses back to source locations; optional `DebugMetadataIndex` retains semantic DWARF scopes and CFI without disrupting baseline debugging when unsupported. `DebugStopContext` captures one paused register state and exposes bounded stack reads to `StackTraceService`, which projects only CFI-verified physical callers and active inline scopes into generation-bound DAP frame IDs. `DapHandleStore` invalidates those frame contexts when execution resumes, restarts, reloads ROM, disconnects, or reaches a newer stop. A physical caller retains its verified resume PC for instruction navigation while source presentation uses a bounded preceding statement in the same subprogram; inline frames use their DWARF call-file, line, and column metadata. A shared debug-source path resolver maps relative and single-rooted DWARF paths against the active project/workspace root for Symbols, Hex Viewer, and DAP stack frames. Shared source navigation reuses visible or inactive open text tabs before creating a preview. ROM/ELF byte mismatches fail metadata loading before source breakpoints can be verified.

`LogicalLocationIndex` groups immutable statement rows into source-step locations by physical frame, inline chain, and source position. `SourceStepService` owns bounded logical-step completion and cancellation; `StepBreakpointStore` reference-counts adapter-owned temporary breakpoints so a source step cannot replace user breakpoint configuration at the same address. `V6DebugAdapter` remains responsible for DAP routing, stop-record precedence, and instruction-level fallback.
...
`ScriptOverlayService` is a connection-scoped retained-delta cache for the optional script-overlay schema. It serializes consuming command `110` reads, rejects responses from inactive generations, and reconciles deletion through explicit successful-operation notifications from `ScriptService`. `EmulatorPanel` owns visible-only polling and sends immutable, crop-adjusted snapshots to a separate transparent Display canvas; `EmulatorSettingsController` owns the global visibility and font-size preferences.
